    ...
    if (f.attributes) {
        ...
        for (var attr in f.attributes) {
            ...
            if (attr!=image_attribute) {
                if (d.search("http") != -1) {
                    h = '<span><strong>' + attr + ':</strong>' + d.link(d) + '</span><br/></br>'; //creates hyperlink by searching for http
                } else {
                    h = '<span><strong>' + attr + ':</strong> ' + d + '</span><br/></br>'; //if no hyperlink found, this just returns the value
                }
            }
