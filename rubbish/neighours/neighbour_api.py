...
class Neighbours():
    ...
    def save(self, filename='neighbourhood_lists.xls', skip=[]):
        ...
        ws.cell(row=1, column=3).value = 'centroid'
        ws.cell(row=1, column=4).value = 'WKT'
        ws.cell(row=1, column=5).value = 'JSON'
...
'LL11', 'LL12', 'LL13', 'LL14', 'LL20', 'SY13', 'SY14']
        ...
        for pcode in districts:
            ...
            if len(first_found):
                ...
                self.save(filename='neighbourhood_lists_{}.xls'.format(pcode))
    ...
    def read_xls_build_geojson(self):
        wb2 = load_workbook('neighbourhood_lists2.xlsx')
...
if __name__ == "__main__":
    nwapi = Neighbours()
    # nwapi.build_grid()
    # nwapi.save(filename='neighbourhood_lists2.xls')
    # nwapi.do_thing()
